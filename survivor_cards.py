...
- **Extraction is conservative** (high-precision patterns only — cashtags, exchange
  ...
  An exact-match hit on a PROSE channel must additionally be CORROBORATED (the 2026-W29
  entity-confusion fix — "Soyuz MS-29"→MS, "Handler AGI"→AGI, "his LI here"→LI, "SMR
  plan"→SMR were all minted as cards): a distinctive registry-name token in the same item,
  or repeated mention across ≥2 distinct CHANNELS that week. Uncorroborated hits are
  DROPPED and counted in a note (see :func:`extract_candidates`) so the operator can spot
  a false negative.
...
# Channels whose item text is PROSE (headlines/posts written by humans): an exact-match
# ticker-string hit there is a string, not ticker intent — the live 2026-W29 card proved a
# stoplist can't cover domain acronyms/proper-noun collisions ("Soyuz MS-29"→Morgan Stanley,
# "Handler AGI"→Alamos Gold, "his LI here"→Li Auto, "SMR plan"→NuScale). Exact-match hits on
# these channels need corroboration (:func:`extract_candidates`); ``orphan_watch`` titles are
# machine-generated by us and stay authoritative. Cashtag/exchange-parenthetical patterns
# carry explicit ticker intent and are exempt.
PROSE_CHANNELS: frozenset[str] = frozenset({"trade_press", "newsletters", "x_lists", "agency"})

# Generic tokens of an SEC registry company name — legal suffixes plus ultra-generic industry
# words — stripped before the distinctive-name-token check, so "NUSCALE POWER CORP"
# corroborates on "nuscale" only (an article merely containing "power" never authenticates
# the SMR string), "URANIUM ENERGY CORP" on "uranium", "ALAMOS GOLD INC" on "alamos"/"gold".
_NAME_GENERIC_TOKENS: frozenset[str] = frozenset({
    "inc", "incorporated", "corp", "corporation", "ltd", "limited", "llc", "lp",
    "holdings", "holding", "group", "plc", "company", "co", "sa", "nv",
    "the", "and", "of",
    "power", "energy", "technologies", "technology", "international", "industries",
    "resources", "systems", "solutions", "services", "partners", "capital",
})
...
def load_known_titles(path: str | Path) -> dict[str, str] | None:
    """symbol → SEC registry title from the same cached ``company_tickers.json`` — the
    name-corroboration side of the known universe (see :func:`extract_candidates`).

    Absent/corrupt cache → ``None`` (mirror of :func:`load_known_tickers`): the name path
    can't fire and single-item exact-match prose hits drop — precision-first fail-closed."""
    p = Path(path)
    if not p.exists():
        return None
    try:
        ...
        out: dict[str, str] = {}
        for r in raw.values():
            out.setdefault(str(r["ticker"]).upper(), str(r.get("title", "")))
        return out
    except Exception:  # noqa: BLE001 — a corrupt cache degrades to no-corroboration
        return None
...
def extract_from_item(item: Item, known: frozenset[str] | None) -> list[Extraction]:
    ...
    This is the RAW per-item pass: exact-match hits on prose channels have NOT yet earned
    their card — :func:`extract_candidates` (the pipeline entry) applies the week-level
    corroboration rule and drops the ambiguous ones, counted."""
...
def _distinctive_name_tokens(title: str | None) -> frozenset[str]:
    """Distinctive tokens of an SEC registry company name: lowercased word tokens minus
    :data:`_NAME_GENERIC_TOKENS`, length ≥ 4 (single-char/short tokens can't corroborate).
    Empty when nothing distinctive remains — such a name can then only corroborate via the
    cross-channel repetition path (precision-first: fewer false cards beats more)."""
    if not title:
        return frozenset()
    toks = _NAME_WORD_RE.findall(title.lower())
...
def extract_candidates(
    ...
    *,
    ...
    with the exact-match CORROBORATION rule applied (the 2026-W29 entity-confusion fix).

    An ``exact_match`` hit on a prose channel (:data:`PROSE_CHANNELS`) is only a
    ticker-shaped string — the live W29 card minted MS (Morgan Stanley) from "Soyuz MS-29",
    AGI (Alamos Gold) from air-cargo handler "Handler AGI", LI (Li Auto) from "his LI here"
    (LinkedIn), and SMR (NuScale) from the reactor acronym. Such a hit survives only if
    corroborated, one of two ways:

    1. **Name corroboration (per item):** a distinctive token of the company's registry
       name (``titles`` — the cached SEC ``company_tickers.json`` title, generic suffixes
       and single-char/generic tokens stripped, remaining token length ≥ 4,
       case-insensitive) appears in the SAME item's title text.
    2. **Cross-channel repetition (per week):** the symbol was surfaced by items in ≥ 2
       distinct CHANNELS this digest week. Deliberately CHANNELS, not items — the live W29
       evidence is that within-channel repetition is the collision NORM, not independent
       mention ("Soyuz MS-29" rode FOUR x_lists items across two accounts; "SMR" rode two
       newsletter items from one source), so distinct-item counting would have kept both.

    Uncorroborated hits are DROPPED and counted into ``notes`` (symbol + a short quote, so
    the operator can spot a false NEGATIVE). Corroboration is per-item: a symbol that
    survives keeps ONLY its corroborated + cashtag/exchange-parenthetical/orphan items in
    provenance (repetition survivors keep all — the items corroborate each other). Honest
    consequence, accepted: a single corporate/insider post saying "for UEC" with no company
    name drops too (a source ACCOUNT's identity can't be auto-known) — the operator can
    cashtag, and the name appears in filings-driven items. Cashtag and exchange-parenthetical
    hits carry explicit ticker intent and are untouched; ``orphan_watch`` titles are
    machine-generated and authoritative."""
    out: dict[str, list[Extraction]] = {}
    for item in items:
        for ex in extract_from_item(item, known):
            out.setdefault(ex.symbol, []).append(ex)

    dropped: dict[str, list[str]] = {}  # symbol → short quotes of dropped items (in order)
    for symbol in list(out):
        exs = out[symbol]
        prose_exact = [ex for ex in exs
        ...
        kept = [ex for ex in prose_exact
                if name_tokens & frozenset(_NAME_WORD_RE.findall(ex.title.lower()))]
        pruned = [ex for ex in prose_exact if ex not in kept]
        if not pruned:
            continue
        dropped[symbol] = [_short_quote(ex.title) for ex in pruned]
        remaining = [ex for ex in exs if ex not in pruned]
        if remaining:
            out[symbol] = remaining
        else:
            del out[symbol]
    if dropped and notes is not None:
        n = sum(len(q) for q in dropped.values())
        parts = []
        for sym in sorted(dropped):
            quotes = dropped[sym]
            extra = f" +{len(quotes) - 1} more" if len(quotes) > 1 else ""
            parts.append(f'{sym}("{quotes[0]}"{extra})')
        notes.append(
            f"{n} ambiguous ticker-string match(es) dropped (no company-name "
            f"corroboration): " + " · ".join(parts)
        )
    return out
