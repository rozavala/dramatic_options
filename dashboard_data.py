...
SCHEMA_EXPECTED = 18  # the highest migration whose data a panel RENDERS — bump ONLY then (not every migration).
#                       Warns only if the DB is BEHIND this (a DB ahead is fine; see header_status schema_ok).
# Rendered through 18: 0013 runs.data_feed (regime_panel) + 0014 gate_dualread (gate_dualread_report)
#                      + 0016 council_proposals.markers_asof (council_l1_health.marker_staleness)
#                      + 0017 cheapness_watch (cheapness_watch_panel) + 0018 null_book_attempts
#                      (null_attempts_panel). 0015 is NOT rendered, but the high-water mark is the DB version.
