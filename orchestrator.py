...
# Configure our own logger with a dedicated handler and disable propagation.
# alpaca-py adds many handlers to the ROOT logger on import; if we propagated,
# every line would be emitted once per root handler (duplicated ~27x). Owning our
# handler keeps output clean regardless of what the SDK does to the root logger.
...
def run_once(cli_live: bool = False) -> int:
    ...
    log.info("Account equity: $%s", f"{equity:,.2f}")
