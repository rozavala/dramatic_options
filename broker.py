...
class Broker(Protocol):
    ...
    def submit_paper(self, *, contract_symbol: str, qty: int, side: str, limit_price: float) -> Fill: ...
...
class AlpacaPaperBroker:
    ...
    def submit_paper(self, *, contract_symbol: str, qty: int, side: str, limit_price: float) -> Fill:
        ...
        if self._dry_run:
            note = f"DRY_RUN (not sent): buy_to_open {qty}x {contract_symbol} LIMIT {limit:.2f} [paper]"
            log.info(note)
            return Fill(True, limit, int(qty), note)  # simulated fill, nothing transmitted

        # Real submission to the PAPER endpoint.
        try:
            ...
            note = f"alpaca submit FAILED for {contract_symbol}: {e}"
            log.error(note)
        ...
        if filled_price:  # immediate fill
            return Fill(True, float(filled_price), int(qty),
                        f"alpaca paper FILLED {qty}x {contract_symbol} @ {float(filled_price):.2f}",
                        order_id=oid)
        # Accepted but resting → pending, reconciled by the monitor.
        return Fill(True, limit, int(qty),
                    f"alpaca paper SUBMITTED {qty}x {contract_symbol} LIMIT {limit:.2f} (resting)",
