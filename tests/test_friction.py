...
def test_friction_inputs_invert_correctly():
    ...
    # zero/None guards → None, not a divide error
    assert friction_inputs(si_pct=None, shares_out=0, adv_usd=None, price=0) == {
        "si_pct": None, "inv_float": None, "inv_adv": None, "inv_price": None
    }
...
def test_corner_is_top_quantile_of_composite():
    ...
    assert all(p == 4 for p in res.inputs_present)  # all four inputs present


def test_mean_imputes_missing_inputs():
    rows = [
        friction_inputs(si_pct=0.5, shares_out=1_000_000, adv_usd=1_000_000, price=2.0),
        {"si_pct": None, "inv_float": None, "inv_adv": None, "inv_price": None},  # all missing
...
def test_input_corr_reported():
    ...
    for i in range(6):
        rows.append(friction_inputs(
            si_pct=0.1 * (i + 1), shares_out=10_000_000 / (i + 1),
    ...
    assert set(FRICTION_INPUTS) == {"si_pct", "inv_float", "inv_adv", "inv_price"}
