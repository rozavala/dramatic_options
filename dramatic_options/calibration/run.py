"""Calibration CLI — PREREG_CONVEXITY_CALIBRATION §6.
    ...
    python -m dramatic_options.calibration.run --mode mc
    python -m dramatic_options.calibration.run --mode mc --paths 20000 --kind C

    # Historical reality-check overlay (caveated; reads the warm bars cache):
    python -m dramatic_options.calibration.run --mode historical --names FCX,RKLB,OKLO
...
# Repo root (parent of the dramatic_options/ package) so `import dramatic_options.*`
# resolves when this is run as a script as well as via `python -m`.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent))

from dramatic_options.calibration.engine import Structure, run_cell_mc  # noqa: E402
from dramatic_options.calibration.metrics import payoff_stats  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.mode == "historical":
        ...
        print("Run the PRIMARY parametric tool: python -m dramatic_options.calibration.run --mode mc")
        return 2
