"""Orchestrator — thematic cheap-convexity paper loop (T1.5).
    ...
    python -m dramatic_options.orchestrator            # one cycle on themes.json (needs Alpaca creds)
    python -m dramatic_options.orchestrator --demo     # one cycle offline on deterministic synthetic data
    python -m dramatic_options.orchestrator --monitor  # mark + apply exits to open positions, NO new entries
    touch KILL && python -m dramatic_options.orchestrator   # halts before any work
...
from dramatic_options.broker import AlpacaPaperBroker, PaperBroker
from dramatic_options.clock import Clock, FixedClock, LiveClock
from dramatic_options.config_loader import (
    ConfigError,
    live_allowed,
    load_config,
...
from dramatic_options.convexity_data import (
    AlpacaChainProvider,
    AlpacaQuoteProvider,
    SyntheticChainProvider,
)
from dramatic_options.council.router import BudgetExceeded, FakeRouter, RouterError, build_router
from dramatic_options.council.wiring import council_to_themes
from dramatic_options.discovery import MarkerParams, scan_baskets
from dramatic_options.monitor import monitor_positions, reconcile_pending
from dramatic_options.paper_loop import kill_rule_status, run_paper_cycle
from dramatic_options.risk import kill_switch_active
from dramatic_options.sentinel_scoring import resolve_due_references
from dramatic_options.state import get_db, record_run
...
def _ensure_schema(conn) -> None:
    ...
    # scripts/ lives at the repo root, one level above this package.
    path = Path(__file__).resolve().parent.parent / "scripts" / "run_migrations.py"
...
def _build_council_io(config: dict, *, demo: bool, client, cache, clock):
    ...
    from dramatic_options.data.news import NewsData
...
def _build_framer_router(config: dict, *, demo: bool, disc: dict):
    ...
    from dramatic_options.council.sentinel import build_framer_router
    try:
    ...
    try:
        ...
        if demo:
            ...
            try:
                ...
                log.error("%s", e)
                return 1
            from dramatic_options.data.alpaca_client import AlpacaClient
        ...
        if result.surfaced:
            ...
            if framer_router is None:
                ...
                from dramatic_options.council.sentinel import frame_candidates
        ...
        if config.get("cluster_diagnostic", {}).get("enabled", True):
            try:
                from dramatic_options import cluster_diagnostic
                from dramatic_options.data.market import MarketData as _MarketData
        ...
        if config.get("basket_quality", {}).get("enabled", True):
            try:
                from dramatic_options import basket_quality
                from dramatic_options.data.market import MarketData as _MarketData
    ...
    try:
        ...
        if demo:
            ...
            try:
                ...
                log.error("%s", e)
                log.error("(No Alpaca creds — use `python -m dramatic_options.orchestrator --demo` for an offline run.)")
                return 1
            from dramatic_options.data.alpaca_client import AlpacaClient
            from dramatic_options.data.cache import PointInTimeCache
